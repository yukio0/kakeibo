...
test('家計簿の操作セルとモバイル表示が大きな金額でも崩れない', async ({ page }) => {
  ...
  const toolbarLayout = await page.locator('.transaction-month-toolbar').evaluate((toolbar) => {
    const items = Array.from(toolbar.children).slice(0, 3)
    const monthInput = toolbar.querySelector<HTMLInputElement>('input[type="month"]')
    const nextButton = items[2]
    if (!monthInput || !nextButton) {
    ...
    return {
      bottoms: items.map((item) => item.getBoundingClientRect().bottom),
      inputToNextGap: nextButtonRect.left - inputRect.right,
    }
  })
  expect(
    Math.max(...toolbarLayout.bottoms) - Math.min(...toolbarLayout.bottoms),
  ).toBeLessThanOrEqual(1)
  expect(toolbarLayout.inputToNextGap).toBeGreaterThanOrEqual(0)
