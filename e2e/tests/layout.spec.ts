...
test('家計簿の操作セルとモバイル表示が大きな金額でも崩れない', async ({ page }) => {
  ...
  for (const width of [480, 390, 320]) {
    ...
    const toolbarLayout = await page.locator('.transaction-month-toolbar').evaluate((toolbar) => {
      const buttons = toolbar.querySelectorAll('button')
      const monthInput = toolbar.querySelector<HTMLInputElement>('input[type="month"]')
      const previousButton = buttons.item(0)
      const nextButton = buttons.item(1)
      if (!monthInput || !previousButton || !nextButton) {
        throw new Error('家計簿の月操作欄が見つかりません')
      }
      const toolbarRect = toolbar.getBoundingClientRect()
      const inputRect = monthInput.getBoundingClientRect()
      ...
      return {
        ...
        previousButton: {
          ...
          right: previousButtonRect.right,
          bottom: previousButtonRect.bottom,
        },
        nextButton: {
          left: nextButtonRect.left,
          top: nextButtonRect.top,
          bottom: nextButtonRect.bottom,
        },
    ...
    expect(
      Math.abs(toolbarLayout.previousButton.top - toolbarLayout.nextButton.top),
    ).toBeLessThanOrEqual(1)
    expect(
      Math.abs(toolbarLayout.previousButton.bottom - toolbarLayout.nextButton.bottom),
    ).toBeLessThanOrEqual(1)
    expect(toolbarLayout.previousButton.right).toBeLessThanOrEqual(toolbarLayout.nextButton.left)
    expect(toolbarLayout.hasHorizontalOverflow).toBeFalsy()
  }
