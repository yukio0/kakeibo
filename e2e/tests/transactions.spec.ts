...
test('スマホで登録・更新した内容をデスクトップ表示にも反映する', async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 })
  await loginThroughMfa(page)

  await page.getByRole('button', { name: '＋ 追加', exact: true }).click()
  let dialog = page.getByRole('dialog')
  await dialog.getByLabel('金額').fill('1000')
  await dialog.getByLabel('メモ').fill('スマホ保存')
  ...
  await expect(page.getByText('登録済み 1件', { exact: true })).toBeVisible()
  await expect(page.locator('.summary-card.expense strong')).toHaveText('￥1,000')

  const mobileEntry = page.locator('.entry-item').filter({ hasText: 'スマホ保存' })
  await expect(mobileEntry).toContainText('￥1,000')
  await mobileEntry.click()

  dialog = page.getByRole('dialog')
  await dialog.getByLabel('金額').fill('1500')
  const updated = page.waitForResponse(
    (response) =>
      response.url().includes('/api/transactions/') &&
      response.request().method() === 'PUT' &&
  ...
  await expect(dialog).not.toBeVisible()
  await expect(page.locator('.summary-card.expense strong')).toHaveText('￥1,500')

  await page.setViewportSize({ width: 1280, height: 900 })
  await expect(enteredRows(page)).toHaveCount(1)
  const desktopRow = enteredRows(page).first()
  await expect(desktopRow.locator('input[type="number"]')).toHaveValue('1500')
  await expect(desktopRow.locator('textarea')).toHaveValue('スマホ保存')
  await expect(desktopRow.getByRole('button', { name: '更新', exact: true })).toBeDisabled()
})
